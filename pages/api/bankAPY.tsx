...
export default async function bankAPY(req : NextApiRequest, res: NextApiResponse){
  ...
  const hasFrozen = new BigNumber( await stakingContract.methods.totalFroze().call() ).isGreaterThan(0)
  // Bankroll
  ...
  const stakingEmission = emission.times( hasFrozen ? boost.plus(1) : 1).times( new BigNumber(1).minus( performanceFee ) ).toNumber() // in wei
  for( 
    let compoundBlock = 1;
