...
export default async function getPrice(req: NextApiRequest, res: NextApiResponse)
{
  ...
  let niceBnb;
  try {
    niceBnb = await new web3.eth.Contract(APE_ABI as AbiItem[], '0x7B83E9530775f8A940a93dFb745dC6c464aD34cf')
  ...
  const totalNice = await niceBnb?.methods.getReserves().call().catch((e: any) => { console.log('crushBnb Error', e); return undefined })
  ...
  const nice1 = new BigNumber(totalNice?._reserve0 || 0).div(new BigNumber(10).pow(18))
  const nBnb1 = new BigNumber(totalNice?._reserve1 || 0).div(new BigNumber(10).pow(18))
  ...
  // GET BNB USD price
  const niceUsdPrice = nBnb1.times(busd2).div(bnb2.times(nice1))
  // GET BNB USD price
  ...
  return res.status(200).json({ message: 'Success Fetching CrushPrice', crushUsdPrice, niceUsdPrice })
}
